...
static bool decode(zvm_vm_t* vm) {
    ...
    switch (vm->cpu.IR->metadata->handler.type) {
        ...
        case ZVM_INST_HANDLER_TYPE_IMM: break;

        case ZVM_INST_HANDLER_TYPE_NONE: break;
    }
...
static bool execute(zvm_vm_t* vm) {
    ...
    instruction_result = handler(vm, vm->cpu.IR->operands[0].value, vm->cpu.IR->operands[1].value, vm->cpu.IR->operands[2].value);
